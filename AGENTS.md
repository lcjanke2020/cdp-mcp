...
- **Breakpoint doesn't bind / `no_mapping` error** → read the error message first: it now says which case you're in — unknown file (echoes the mapped source paths, same-basename candidates first), mapped file but unmapped line (suggests the nearest mapped lines), or no source maps loaded yet (GH #37). For the deep dive, `list_scripts` (which scripts loaded?) and `resolve_source_position` (where does the source map place this line?). The `mapOriginalToGenerated` comment in `src/sourcemap/store.ts` explains why `allGeneratedPositionsFor` is required.
