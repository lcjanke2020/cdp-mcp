...
| `harness/deepseek-adapter.ts` | DeepSeek adapter — `makeDeepseekAdapter()` (GH #8). Thin wrapper over the factory; reads `EVAL_DEEPSEEK_*`, defaults to `https://api.deepseek.com/v1`. Use the v4 model ids. Reasoning on via `thinking`:{type:enabled} + top-level `reasoning_effort:high` (override per run via `EVAL_DEEPSEEK_REASONING_EFFORT`; output cap via `EVAL_DEEPSEEK_MAX_TOKENS` — GH #7) — `reasoning_content` captured to the `.thinking` sidecar AND re-fed on tool-call turns (V4 requires it echoed back, same as Kimi — verified vs the live API). Cache via top-level `prompt_cache_hit_tokens`. |
| `harness/moonshot-adapter.ts` | Moonshot (Kimi) adapter — `makeMoonshotAdapter()` (GH #8). Thin wrapper over the factory; reads `EVAL_MOONSHOT_*`, defaults to the global `https://api.moonshot.ai/v1`. The eval `/v1` path — distinct from the Kimi Claude Code `/anthropic` setup. K2 reasons by server-side default (no request-side effort knob, so no reasoning-effort env either); output cap overridable via `EVAL_MOONSHOT_MAX_TOKENS` (GH #7). `reasoning_content` is captured AND re-fed (K2 hard-rejects a tool-call turn that omits it). Cache via `prompt_tokens_details.cached_tokens`. |
...
| `harness/model.ts` | Default model (`claude-opus-4-8` + adaptive `medium` thinking), per-vendor pricing catalog (`PRICING_CATALOG` + `pricingFor(vendor, model)` post-#48; LM Studio wildcard `"*"` sentinel = $0; DeepSeek + Kimi rows added in GH #8, each with an `inputCacheRead` bucket so `estimateCostUsd` credits the cache-read discount), reasoning config, env overrides (`EVAL_MODEL_OVERRIDE`, `EVAL_REASONING_LEVEL`, `EVAL_REASONING_BUDGET`). |
...
|---|---|---|
| `node-compute-step` | `compute-step.ts` | Canonical bp/pause/inspect cycle; `npm run eval:quick:node` smoke target. `get_node_output` is optional/diagnostic — mechanic comes from the breakpoint-hit cycle. |
| `node-stdio-bug` | `stdio-bug.ts` | Mandates `get_node_output` — the wrong value is observable only via the OS-pipe stdio buffer, not via V8's `Runtime.consoleAPICalled` console buffer. |
| `node-conditional-bp` | `conditional-bp.ts` | Conditional-breakpoint primitive — the bp sits on a line every iteration runs, so the oracle only passes when the `condition` is actually applied. |
| `node-uncaught-throw` | `throw.ts` | Exception flow + re-launch recovery — the oracle iterates ALL pause indices so a re-launch that re-pauses on the same throw still replays the inspection. |
...
# Cross-vendor: DeepSeek + Kimi/Moonshot (GH #8 — remote OpenAI-compat
...
1. Add `scenarios/<name>.ts`: export a `Scenario` (from `harness/types.ts`) with:
   ...
   - Target — one of:
     ...
     - *Dual:* `target: { kind: "dual", webAppDir, webUrl, script }`. The runner starts the Vite CLI under `webAppDir`, passes `webUrl` plus the backend `script` to the agent, and tears the dev server down. `webUrl` is the exact page URL the readiness probe checks and the browser opens; paths, queries, and fragments are allowed, while Vite's bind host/port are derived from that URL. Use `fullstack-cart.ts` as the canonical example and add a dedicated fixture-build script when introducing another app.
